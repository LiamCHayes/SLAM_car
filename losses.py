...
class Reward:
    ...
    def discover_reward(self, sim: simulator.Simulator):
        ...
        def is_in_map(row, col):
            """Returns true if is in car map as well as coordinates in the car map"""
    ...
    def collect_reward(self, done, sim):
        """
...
def pct_area_explored(sim: simulator.Simulator):
    """
    Returns a negative percent of the area explored
    """
    loss = -np.round(np.sum(sim.car.map >= 0) / np.sum(sim.simulated_map.map >= 0) * 100, 2)
    return loss